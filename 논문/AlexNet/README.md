...
|------|-----------|-----------|------|
| 입력 이미지 | — | `(1, 3, 224, 224)` | RGB 3채널, 224px |
| Conv1 (11×11, s=4) | `(1, 3, 224, 224)` | `(1, 96, 55, 55)` | (224-11)/4+1=55, 필터 96개 |
| LRN + MaxPool | `(1, 96, 55, 55)` | `(1, 96, 27, 27)` | (55-3)/2+1=27, 채널 유지 |
| Conv2 (5×5) | `(1, 96, 27, 27)` | `(1, 256, 27, 27)` | padding=2로 크기 유지, 필터 증가 |
| LRN + MaxPool | `(1, 256, 27, 27)` | `(1, 256, 13, 13)` | (27-3)/2+1=13 |
| Conv3 (3×3) | `(1, 256, 13, 13)` | `(1, 384, 13, 13)` | padding=1로 크기 유지, 필터 증가 |
| Conv4 (3×3) | `(1, 384, 13, 13)` | `(1, 384, 13, 13)` | 크기·채널 수 동일 유지 |
| Conv5 (3×3) | `(1, 384, 13, 13)` | `(1, 256, 13, 13)` | 필터 수 384→256으로 감소 |
| MaxPool | `(1, 256, 13, 13)` | `(1, 256, 6, 6)` | (13-3)/2+1=6 |
| Flatten | `(1, 256, 6, 6)` | `(1, 9216)` | 256×6×6=9216, 1D로 펼침 |
| FC1 + Dropout | `(1, 9216)` | `(1, 4096)` | 선형 변환, 뉴런 50% 무작위 꺼짐 |
| FC2 + Dropout | `(1, 4096)` | `(1, 4096)` | 크기 유지, Dropout 적용 |
| FC3 (출력층) | `(1, 4096)` | `(1, 1000)` | 클래스 수 1000개에 맞게 축소 |
| Softmax (최종 출력) | `(1, 1000)` | `(1, 1000)` | 확률값으로 변환 (합계=1.0) |
